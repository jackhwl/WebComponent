...
function twentyfour() {
	...
	return movieLists.
		concatMap(movieList => movieList.videos.
         concatMap(video => Array.zip(video.boxarts.
              reduce((pre, cur) => pre.width * pre.height < cur.width * cur.height ? pre : cur),
      				video.interestingMoments.
                filter(interestingmoment => interestingmoment.type==='Middle'), 
                  (boxart, moment) => ({id: video.id, title: video.title, time: moment.time, url: boxart.url }))
                  )
    );
